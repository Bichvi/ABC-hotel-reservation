...
// TÍNH GIÁ
// ==========================
$giaPhong = (float)($room['Gia'] ?? 0);
$soDem    = max(1, (strtotime($booking['ngay_tra'] ?? 'now') - strtotime($booking['ngay_nhan'] ?? 'now')) / 86400);

$tienGoc   = $summary['tong_tien']   ?? ($giaPhong * $soDem);
$giamGia   = $summary['giam_gia']    ?? 0;
$tongSauKM = $summary['tong_sau_km'] ?? ($tienGoc - $giamGia);
    ...
    <div class="card-shell">
        ...
        <div class="summary-box">
            ...
            <?php else: ?>
                <div class="summary-total">
                    <span>Tổng tiền</span>
                    <strong><?= number_format($tienGoc) ?> đ</strong>
                </div>
            <?php endif; ?>
        </div>
        ...
        <form method="POST" action="index.php?controller=khachhang&action=confirmBooking">
            ...
            <!-- ⭐ PHẢI GỬI MÃ KM SANG CONFIRM -->
            <input type="hidden" name="booking[ma_km]" value="<?= e($summary['ma_km'] ?? 0) ?>">
