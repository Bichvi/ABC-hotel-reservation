        ...
        <form method="post"
            ...
            <div class="mb-3">
                ...
                <textarea
                    ...
                    class="form-control"
                    maxlength="150"
                    rows="3"><?= htmlspecialchars($old['DiaChi'] ?? '') ?></textarea>
                <div class="d-flex justify-content-between align-items-center mt-1">
                    <div id="err_DiaChi" class="text-error-inline">
                        Đã đạt giới hạn 150 ký tự.
                    </div>
                    <div class="text-muted small">
                        <span id="diachiCount"><?= isset($old['DiaChi']) ? mb_strlen($old['DiaChi']) : 0 ?></span>/150
                    </div>
                </div>
            </div>
...
<script>
    ...
    // Địa chỉ: cập nhật bộ đếm và hiển thị lỗi ngay khi đạt 150 ký tự
    const elDiaChi = document.getElementById('DiaChi');
    const diachiCount = document.getElementById('diachiCount');
    const errDiaChi = document.getElementById('err_DiaChi');

    function updateDiachiCounter() {
        if (!elDiaChi) return;
        const v = elDiaChi.value || '';
        const len = v.length;
        if (diachiCount) diachiCount.textContent = len;

        if (len >= 150) {
            elDiaChi.classList.add('is-invalid');
            if (errDiaChi) errDiaChi.classList.add('active');
        } else {
            elDiaChi.classList.remove('is-invalid');
            if (errDiaChi) errDiaChi.classList.remove('active');
        }
    }

    if (elDiaChi) {
        elDiaChi.addEventListener('input', function (ev) {
            // đảm bảo không vượt quá 150 ký tự (vì maxlength nên hiếm khi xảy ra, nhưng phòng paste)
            if (elDiaChi.value.length > 150) {
                elDiaChi.value = elDiaChi.value.slice(0, 150);
            }
            updateDiachiCounter();
        });
        // khởi tạo bộ đếm ban đầu
        updateDiachiCounter();
    }
    ...
    document.getElementById('frm-khachhang').addEventListener('submit', function (e) {
        ...
        if (typeof elDiaChi !== 'undefined') elDiaChi.dispatchEvent(new Event('input'));
