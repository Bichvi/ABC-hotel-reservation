...
<nav class="navbar navbar-expand-lg navbar-dark border-bottom border-slate-700">
    <div class="container d-flex justify-content-between">
        <a class="navbar-brand" href="index.php?controller=dashboard&action=cskh">
    ...
    <div class="wrapper">
            <div class="action-bar d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom border-secondary">
                ...
                <?php else: ?>
                    <a href="index.php?controller=cskh&action=phanHoi" class="btn-back">
        ...
        <?php if ($mode === "list"): ?>
            ...
            <input type="hidden" name="controller" value="cskh">
            ...
            <div class="d-flex gap-2">
                ...
                <?php if(!empty($keyword) || !empty($status) || !empty($rating) || !empty($dateFilter)): ?>
                    <a href="index.php?controller=cskh&action=phanHoi" class="btn btn-danger fw-bold px-4">
                ...
                <table class="table table-dark table-hover table-custom">
                    ...
                    <tbody>
                        <?php foreach ($dsPhanHoi as $ph): ?>
                            ...
                            ?>
                            <tr style="cursor:pointer" onclick="location.href='index.php?controller=cskh&action=chiTietPhanHoi&id=<?= $ph['MaPH'] ?>'">
...
<?php if (!empty($history)): ?>
                ...
                <?php if ($_GET['msg'] === 'success'): ?>
                    <div class="alert mb-4 text-center" style="background: linear-gradient(135deg, rgba(16, 185, 129, 0.2), rgba(5, 150, 105, 0.2)); border: 2px solid #10b981; border-radius: 15px; padding: 30px;">
                        ...
                        <div class="d-flex gap-2 justify-content-center">
                            <button type="button" class="btn btn-outline-light px-4" style="border-radius: 8px; font-weight: 600;" onclick="window.location.href='index.php?controller=cskh&action=chiTietPhanHoi&id=<?= $info['MaPH'] ?>'">
                                <i class="fa-solid fa-rotate-left me-2"></i>Gửi phản hồi khác
                            </button>
                            <a href="index.php?controller=cskh&action=phanHoi" class="btn px-4" style="border-radius: 8px; font-weight: 600; background: linear-gradient(135deg, #10b981, #059669); color: white; border: none;">
            ...
            <form action="index.php?controller=cskh&action=luuPhanHoi" method="POST" id="formPhanHoi">
                ...
                <div class="detail-box" style="border-color: #38bdf8;">
                    ...
                    <div id="charLimitError" class="small text-danger mt-1" style="display:none">Không được nhập quá 500 ký tự.</div>
                    <?php if (isset($_GET['error']) && $_GET['error'] === 'toolong'): ?>
                        <div class="small text-danger mt-1">Nội dung quá dài (tối đa 500 ký tự).</div>
                    <?php endif; ?>
                    ...
                    <div class="text-center">
                        ...
                        <a href="index.php?controller=cskh&action=phanHoi" class="btn btn-outline-secondary px-4 py-2">Hủy</a>
                    </div>
                </div>
            </form>
        ...
        <div class="modal-content" style="background: linear-gradient(135deg, rgba(16, 185, 129, 0.95), rgba(5, 150, 105, 0.95)); border: 2px solid rgba(52, 211, 153, 0.6); border-radius: 15px; box-shadow: 0 20px 60px rgba(16, 185, 129, 0.4);">
            <div class="modal-header border-0 pb-0">
                <h5 class="modal-title text-white fw-bold">
                    <i class="fa-solid fa-paper-plane fa-bounce me-2"></i>Xác nhận gửi phản hồi
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body text-white" style="font-size: 15px;">
                <p class="mb-0">
                    <i class="fa-solid fa-circle-info me-2"></i>
                    Bạn có chắc chắn muốn gửi phản hồi này cho khách hàng?
                </p>
                <p class="mb-0 mt-2 text-white-50" style="font-size: 13px;">
            ...
            <div class="modal-footer border-0 pt-0">
                ...
                <button type="button" class="btn px-4 fw-bold" onclick="submitForm()" style="background: linear-gradient(135deg, #fbbf24, #f59e0b); color: #1f2937; border: none; border-radius: 8px;">
                    <i class="fa-solid fa-paper-plane me-2"></i>Xác nhận gửi
                </button>
            </div>
        </div>
    </div>
...
function showConfirmModal() {
    ...
    // Kiểm tra độ dài ký tự (client-side) - tính Enter = 2 ký tự
    const _s = noiDung.value || '';
    const _newlines = (_s.match(/\n/g) || []).length;
    const len = _s.length + _newlines;
    if (len > MAX_CHARS) {
        // Show small inline error and update counter, do not show bottom big error
        ...
        if (charCountInfo) { charCountInfo.classList.remove('text-white'); charCountInfo.classList.add('text-danger'); }
        return;
    }

    if (errors.length > 0) {
        // Hiển thị cảnh báo trên
        errorText.textContent = 'Kiểm tra và điền đầy đủ thông tin!';
        errorDiv.style.display = 'block';

        // Hiển thị chi tiết lỗi dưới
        errorTextBottom.innerHTML = '<ul class="mb-0 ps-3">' + errors.map(e => '<li>' + e + '</li>').join('') + '</ul>';
...
if (noiDungInput && charCountInfo) {
    ...
    // Treat Enter (newline) as 2 characters for counting to help users avoid host-length issues
    ...
    function truncateToLogical(text, max) {
        if (!text) return '';
        let out = '';
        let len = 0;
    ...
    function updateCharCountAndEnforce() {
        const logicalLen = countChars(noiDungInput.value);
        if (logicalLen >= MAX_CHARS) {
            noiDungInput.value = truncateToLogical(noiDungInput.value, MAX_CHARS);
            const displayLen = countChars(noiDungInput.value);
            charCountInfo.textContent = displayLen + '/' + MAX_CHARS + ' ký tự';
            if (charLimitError) charLimitError.style.display = 'block';
            charCountInfo.classList.remove('text-white');
            charCountInfo.classList.add('text-danger');
            // hide big bottom error
            const errBottom = document.getElementById('errorMessageBottom'); if (errBottom) errBottom.style.display = 'none';
        } else {
            charCountInfo.textContent = logicalLen + '/' + MAX_CHARS + ' ký tự';
            if (charLimitError) charLimitError.style.display = 'none';
            charCountInfo.classList.remove('text-danger');
            charCountInfo.classList.add('text-white');
        }
        // hide general errors on typing
        const errTop = document.getElementById('errorMessage'); if (errTop) errTop.style.display = 'none';
        const errBottom = document.getElementById('errorMessageBottom'); if (errBottom) errBottom.style.display = 'none';
    }
    ...
    // paste handling (respect logical limit where Enter = 2 chars)
    noiDungInput.addEventListener('paste', function(e) {
        ...
        noiDungInput.value = truncateToLogical(newVal, MAX_CHARS);
        updateCharCountAndEnforce();
    });
...
// Ẩn lỗi khi chọn trạng thái
const statusRadios = document.getElementsByName('TinhTrang');
