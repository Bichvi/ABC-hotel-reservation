...
class KhachhangController extends Controller
{
   ...
   public function huyDatPhong()
    {
        ...
        // Lấy danh sách phòng đang đặt
        $sql = "
            SELECT gd.MaGiaoDich, ct.MaCTGD, ct.MaPhong, p.SoPhong, p.LoaiPhong,
                ct.NgayNhanDuKien, ct.NgayTraDuKien, ct.TrangThai
            FROM giaodich gd
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["btn_action"] ?? "") === "cancel_room") {
            ...
            // Lấy ngày nhận từ DB
            $sqlDate = "SELECT NgayNhanDuKien FROM chitietgiaodich WHERE MaCTGD = ?";
            $stmtDate = $db->prepare($sqlDate);
            $stmtDate->bind_param("i", $maCTGD);
            $stmtDate->execute();
            $ngayNhanRaw = $stmtDate->get_result()->fetch_column();

            if (!$ngayNhanRaw) {
                $errors[] = "Không tìm thấy ngày nhận phòng!";
            } else {

                date_default_timezone_set("Asia/Ho_Chi_Minh");
                $today = new DateTime(date("Y-m-d"));
                $ngayNhan = new DateTime($ngayNhanRaw); // DB dạng Y-m-d

                // ❗ Chỉ cho hủy nếu hôm nay < ngày nhận
                if ($today >= $ngayNhan) {
                    $errors[] = "Không thể hủy vì đã quá hạn check-in!";
                }
            }
            ...
            if (empty($errors)) {
                ...
                try {
                    ...
                    // Hủy phòng
                    $db->query("UPDATE chitietgiaodich SET TrangThai='DaHuy' WHERE MaCTGD=$maCTGD");

                    // Trả phòng về trạng thái trống
                    $db->query("UPDATE phong SET TrangThai='Trong' WHERE MaPhong=$maPhong");

                    // Kiểm tra còn phòng nào đang Booked trong đơn không
                    $sqlCheck = "
                        SELECT COUNT(*) AS c 
                        FROM chitietgiaodich
                        WHERE TrangThai!='DaHuy'
                        AND MaGiaoDich=(SELECT MaGiaoDich FROM chitietgiaodich WHERE MaCTGD=$maCTGD LIMIT 1)
                    ";
                    $check = $db->query($sqlCheck)->fetch_assoc()['c'];

                    if ($check == 0) {
                        $db->query("
                            UPDATE giaodich 
                            SET TrangThai='DaHuy'
                            WHERE MaGiaoDich=(SELECT MaGiaoDich FROM chitietgiaodich WHERE MaCTGD=$maCTGD LIMIT 1)
                        ");
                    }
        ...
        $this->view("khachhang/huy_dat_phong", [
            "bookings" => $bookings,
            "errors"   => $errors,
            "success"  => $success
        ]);
   ...
   public function datPhongOnline1()
    ...
    public function datPhongOnline2()
    {
        ...
        return $this->view("khachhang/dat_phong_online1", compact("room"));
    }
}